...
public class VentasGateway extends RcbikeRestGateway {
	...
	public Long crearVenta(VentaWeb venta) {
		return webTarget().path(OperacionesVentas.EP_VENTA)
				.path(OperacionesVentas.EP_VENTA).request()
	...
	public List<VentaWeb> listPublicacionesByEmail() {
		return webTarget().path(OperacionesVentas.EP_VENTA)
				.path(OperacionesVentas.VENTAS).request().get(TYPE_LIST_VENTAS);
	}

	public void deleleVentas(Long id) {
		webTarget().path(OperacionesVentas.EP_VENTA)
				.path(OperacionesVentas.EP_VENTA).path(id + "").request()
				.delete();
	}
}
